...
1. **Complaints** — sourced from a local sliced CSV (~12.8M rows, ~5-year
   window). The slice is generated by `scripts/slice_complaints.py` from the
   CFPB bulk download.
2. **Date** — generated in Power Query using `List.Dates`, required for
   time-intelligence DAX functions. Currently spans 2024-01-01 to today;
   widening to 2011-onward to match the Complaints range is a known follow-up.
